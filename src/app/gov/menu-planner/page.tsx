...
import { findTopVendors, VendorResponse } from './vendor-finder';
...
export default function MenuPlannerPage() {
  ...
  const [foundVendors, setFoundVendors] = useState<VendorResponse>();
  ...
  const searchVendors = async () => {
    ...
    setVendorError('');
    try {
      const vendorData = await findTopVendors({
        province: currentProvince,
        budget,
        distance,
      });
      setFoundVendors(vendorData);
    } catch (error) {
      setVendorError(
        `Error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
  ...
  const getPriceForDate = (date: number) => {
    if (generatedMenus?.daily_menus) {
      const menu = generatedMenus.daily_menus.find(
        (m: { date: number; price_per_portion?: number }) => m.date === date
      );
    ...
    <div className='space-y-8'>
      <div className='rounded-lg p-8 flex flex-col justify-center items-center w-full'>
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(to right, #02B5AC, #D2DD25)',
          }}
        />
      ...
      <div className='bg-white rounded-lg shadow-lg p-8'>
        <div className='flex justify-between items-center mb-6'>
          <div className='flex items-center gap-3'>
            ...
            />
            <h2 className='text-2xl font-semibold text-gray-900'>
              Top 5 Vendor Kitchen MBG
            </h2>
          </div>
          <button
            onClick={searchVendors}
            disabled={isSearchingVendors}
            className='bg-secondary text-white px-6 py-3 text-base font-semibold rounded-lg hover:bg-secondary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2'
          >
            <IconSearch size={20} />
            {isSearchingVendors ? 'Searching...' : 'Cari Vendor'}
          </button>
        </div>
        ...
        {vendorError && (
          <div className='mb-6 bg-red-50 p-4 rounded-lg border border-red-200'>
            ...
            <p className='text-red-700 whitespace-pre-wrap'>{vendorError}</p>
          </div>
        )}
        ...
        {foundVendors ? (
          <div className='space-y-4'>
            <div className='bg-blue-50 p-4 rounded-lg border border-blue-200 mb-6'>
              <p className='text-sm text-blue-800'>
                ...
                {foundVendors.search_summary.total_found} vendors found
              </p>
              <p className='text-sm text-blue-700 mt-1'>
                {foundVendors.search_summary.search_criteria}
              </p>
              <p className='text-xs text-blue-600 mt-1'>
                Last updated: {foundVendors.search_summary.last_updated}
              </p>
            </div>

            <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
              {foundVendors.vendors.map((vendor, index) => (
                <div
                  key={index}
                  className='border-2 border-gray-200 rounded-lg p-6 hover:border-primary/50 transition-colors'
                >
                  ...
                  <div className='space-y-3'>
                    ...
                    <div>
                      ...
                      <div className='flex flex-wrap gap-1'>
                        {vendor.certification.map((cert, idx) => (
                          <span
                            key={idx}
                    ...
                    <div className='grid grid-cols-2 gap-4 pt-3 border-t border-gray-200'>
                      <div>
                        <p className='text-xs text-gray-500'>Kapasitas/Hari</p>
                        <p className='text-sm font-bold text-gray-900'>
                          {vendor.capacity_per_day.toLocaleString('id-ID')}{' '}
                          porsi
                        </p>
                      </div>
                      <div>
                        ...
                        <p className='text-sm font-bold text-gray-900'>
                          Rp {formatCurrency(vendor.estimated_cost_per_portion)}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className='text-center py-12 text-gray-500'>
            ...
            />
            <p className='text-lg'>
              Klik "Cari Vendor" untuk mencari kitchen vendor terbaik di{' '}
              {currentProvince}
            </p>
            <p className='text-sm mt-2'>
              Sistem akan mencari vendor yang sesuai dengan budget Rp{' '}
              {formatCurrency(budget)} dan jarak maksimal {distance} km
            </p>
          </div>
        )}
      </div>
